...
The supported models can be displayed using the following command
```{r}
show_learners()
```
...
# install.packages("rpart")  ## required to install rpart, if the package was not installed from GitHub

# library(rpart) # check if this is required !!!
...
# Assumption: .libPaths(c("/scratch/pradap/r-libs/", .libPaths())) 
# is already executed in the curret R - session
#install.packages("randomForest") ## required to install randomForest if the package was not installed from GitHub
...
# Assumption: .libPaths(c("/scratch/pradap/r-libs/", .libPaths())) 
# is already executed in the curret R - session
# install.packages("e1071") ## required to install e1071, if the package was not installed from GitHub
library(e1071)
